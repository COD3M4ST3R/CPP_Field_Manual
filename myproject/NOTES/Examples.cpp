...
public:
    ...
    explicit ComplexClassUsesHeap(size_t p_size)
        : m_data(new int[p_size]), m_dataSize(p_size)
    {
...
/*
template <typename T>
inline void data_displayer(const std::unique_ptr<std::vector<T>> &dataContainer) // takes the reference of the smart pointer that points the type T vector
